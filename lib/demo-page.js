...
const cssClass = (fontFamily, props) => `
	.${fontFamily}-${props.style}-${props.weight} {
		font-family: ${fontFamily};
		font-style: ${props.style};
		font-weight: ${props.weight};
	}
			...
			<style>
				...
				${Object.entries(fonts)
					.map(([fontFamily, familyFonts]) =>
						familyFonts
							.map((familyFont) =>
								Object.values(familyFont)
									.map((props) => cssClass(fontFamily, props))
									.join(''),
							)
							.join(''),
					)
					...
					familyFonts
						.map((familyFont) =>
							Object.entries(familyFont)
								.map(
									([name, props]) =>
										`<span class="${fontFamily}-${props.style}-${props.weight}">${name}</span>`,
								)
								.join('\n'),
						)
						.join(''),
